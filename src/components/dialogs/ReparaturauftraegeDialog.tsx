...
import type { Reparaturauftraege, Kunden, LookupValue } from '@/types/app';
...
interface ReparaturauftraegeDialogProps {
  ...
  defaultValues?: Omit<Reparaturauftraege['fields'], 'status'> & {
...
export function ReparaturauftraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: ReparaturauftraegeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['auftragsnummer', 'kunde', 'hersteller', 'modell', 'fehlerbeschreibung', 'zusagedatum', 'status'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "auftragsnummer": string | null, // Auftragsnummer\n  "kunde": string | null, // Display name from Kunden (see <available-records>)\n  "hersteller": string | null, // Hersteller\n  "modell": string | null, // Modell\n  "fehlerbeschreibung": string | null, // Fehlerbeschreibung\n  "zusagedatum": string | null, // YYYY-MM-DD\n  "status": LookupValue | null, // Status (select one key: "angenommen" | "in_reparatur" | "wartet_auf_teile" | "fertig") mapping: angenommen=Angenommen, in_reparatur=In Reparatur, wartet_auf_teile=Wartet auf Teile, fertig=Fertig\n  "heute_faellig": boolean | null, // Heute fällig\n  "interne_notizen": string | null, // Interne Notizen\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Reparaturaufträge bearbeiten' : 'Reparaturaufträge hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'hersteller': (
      <div key="hersteller" className="space-y-1.5">
        <Label htmlFor="hersteller">Hersteller <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="hersteller"
          placeholder="z. B. Bosch, Makita, DeWalt"
          value={fields.hersteller ?? ''}
          onChange={e => setFields(f => ({ ...f, hersteller: e.target.value }))}
          required
        />
        {showErrors && !fields.hersteller && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'modell': (
      <div key="modell" className="space-y-1.5">
        ...
        <Input
          id="modell"
          placeholder="z. B. GSR 18V-21"
          value={fields.modell ?? ''}
          onChange={e => setFields(f => ({ ...f, modell: e.target.value }))}
          required
        />
        {showErrors && !fields.modell && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'fehlerbeschreibung': (
      <div key="fehlerbeschreibung" className="space-y-1.5">
        <Label htmlFor="fehlerbeschreibung">Fehlerbeschreibung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="fehlerbeschreibung"
          placeholder="Was ist defekt? Symptome, Fehlercode, Verhalten..."
          value={fields.fehlerbeschreibung ?? ''}
          onChange={e => setFields(f => ({ ...f, fehlerbeschreibung: e.target.value }))}
          rows={3}
        />
        {showErrors && !fields.fehlerbeschreibung && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'zusagedatum': (
      <div key="zusagedatum" className="space-y-1.5">
        <Label htmlFor="zusagedatum">Zusagedatum <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="zusagedatum"
          placeholder="Wann soll es fertig sein?"
          mode="date"
          value={fields.zusagedatum ?? null}
          onChange={v => setFields(f => ({ ...f, zusagedatum: v ?? undefined }))}
          required
        />
        {showErrors && !fields.zusagedatum && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'in_reparatur'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'in_reparatur' ? undefined : 'in_reparatur') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'in_reparatur'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'wartet_auf_teile'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'wartet_auf_teile' ? undefined : 'wartet_auf_teile') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'wartet_auf_teile'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'heute_faellig': (
      <div key="heute_faellig" className="space-y-1.5">
        <Label htmlFor="heute_faellig">Heute fällig</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="heute_faellig"
            checked={!!fields.heute_faellig}
            onCheckedChange={(v) => setFields(f => ({ ...f, heute_faellig: !!v }))}
          />
          <Label htmlFor="heute_faellig" className="font-normal">Heute fällig</Label>
        </div>
      </div>
    ),
    'interne_notizen': (
      <div key="interne_notizen" className="space-y-1.5">
        ...
        <Textarea
          id="interne_notizen"
          placeholder="Reparaturschritte, Teile, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"auftragsnummer": "Auftragsnummer", "kunde": "Kunde", "hersteller": "Hersteller", "modell": "Modell", "fehlerbeschreibung": "Fehlerbeschreibung", "zusagedatum": "Zusagedatum", "status": "Status", "heute_faellig": "Heute fällig", "interne_notizen": "Interne Notizen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"email": "E-Mail-Adresse", "vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer"}};

...
const REPARATURAUFTRAEGE_FIELDS = [
  ...
  { key: 'fehlerbeschreibung', label: 'Fehlerbeschreibung', type: 'string/textarea' },
  { key: 'zusagedatum', label: 'Zusagedatum', type: 'date/date' },
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'angenommen', label: 'Angenommen' }, { key: 'in_reparatur', label: 'In Reparatur' }, { key: 'wartet_auf_teile', label: 'Wartet auf Teile' }, { key: 'fertig', label: 'Fertig' }] },
  { key: 'heute_faellig', label: 'Heute fällig', type: 'bool' },
...
const ENTITY_TABS = [
  { key: 'reparaturauftraege', label: 'Reparaturaufträge', pascal: 'Reparaturauftraege' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('reparaturauftraege');
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'reparaturauftraege': return REPARATURAUFTRAEGE_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'reparaturauftraege': return {
        create: (fields: any) => LivingAppsService.createReparaturauftraegeEntry(fields),
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'reparaturauftraege' || dialogState?.entity === 'reparaturauftraege') && (
        <ReparaturauftraegeDialog
          open={createEntity === 'reparaturauftraege' || dialogState?.entity === 'reparaturauftraege'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'reparaturauftraege' ? handleUpdate : (fields: any) => handleCreate('reparaturauftraege', fields)}
          defaultValues={dialogState?.entity === 'reparaturauftraege' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'reparaturauftraege' && (
        <ReparaturauftraegeViewDialog
          open={viewState?.entity === 'reparaturauftraege'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'reparaturauftraege', record: r }); }}
          kundenList={(data as any).kunden ?? []}
        />
      )}

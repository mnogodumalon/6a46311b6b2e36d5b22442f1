...
export default function ReparaturauftraegePage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Reparaturauftraege[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Reparaturauftraege | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Reparaturauftraege | null>(null);
  ...
  async function handleCreate(fields: Reparaturauftraege['fields']) {
  ...
  async function handleUpdate(fields: Reparaturauftraege['fields']) {
    ...
    await LivingAppsService.updateReparaturauftraegeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteReparaturauftraegeEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Reparaturaufträge im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Reparaturaufträge suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('fehlerbeschreibung')}>
                <span className="inline-flex items-center gap-1">
                  Fehlerbeschreibung
                  {sortKey === 'fehlerbeschreibung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zusagedatum')}>
                <span className="inline-flex items-center gap-1">
                  Zusagedatum
                  {sortKey === 'zusagedatum' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('heute_faellig')}>
                <span className="inline-flex items-center gap-1">
                  Heute fällig
                  {sortKey === 'heute_faellig' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/reparaturauftraege/${record.record_id}`); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.fehlerbeschreibung ?? '—'}</span></TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.zusagedatum)}</TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.heute_faellig ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.heute_faellig ? 'Ja' : 'Nein'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={10} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Reparaturaufträge. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>

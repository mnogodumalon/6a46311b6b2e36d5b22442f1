...
import type { Reparaturauftraege, Kunden, CreateReparaturauftraege, CreateKunden } from '@/types/app';
...
export class LivingAppsService {
  // --- REPARATURAUFTRAEGE ---
  static async getReparaturauftraege(): Promise<Reparaturauftraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.REPARATURAUFTRAEGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Reparaturauftraege[];
    return enrichLookupFields(records, 'reparaturauftraege');
  }
  static async getReparaturauftraegeEntry(id: string): Promise<Reparaturauftraege | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.REPARATURAUFTRAEGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Reparaturauftraege;
    return enrichLookupFields([record], 'reparaturauftraege')[0];
  }
  static async createReparaturauftraegeEntry(fields: CreateReparaturauftraege) {
    return callApi('POST', `/apps/${APP_IDS.REPARATURAUFTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'reparaturauftraege') });
  }
  static async updateReparaturauftraegeEntry(id: string, fields: Partial<CreateReparaturauftraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.REPARATURAUFTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reparaturauftraege') });
  }
  static async deleteReparaturauftraegeEntry(id: string) {
  ...
  static async createKundenEntry(fields: CreateKunden) {
    return callApi('POST', `/apps/${APP_IDS.KUNDEN}/records`, { fields: cleanFieldsForApi(fields as any, 'kunden') });
  }
  static async updateKundenEntry(id: string, fields: Partial<CreateKunden>) {
    return callApi('PATCH', `/apps/${APP_IDS.KUNDEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kunden') });
  }

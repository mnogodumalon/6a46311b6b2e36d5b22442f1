...
export function useDashboardData() {
  ...
  const fetchAll = useCallback(async () => {
    setError(null);
    try {
      const [reparaturauftraegeData, kundenData] = await Promise.all([
  ...
  return { reparaturauftraege, setReparaturauftraege, kunden, setKunden, loading, error, fetchAll, kundenMap };
}